...
// ─── Decorative QR (same pattern as KioskPanel) ───────────────────────────────
...
function TicketQR({ dim }: { dim: number }) {
  return (
    <svg width={dim} height={dim} viewBox={`0 0 ${QR_N} ${QR_N}`} shapeRendering="crispEdges">
      {QR_CELLS.map((f, i) =>
        f ? <rect key={i} x={i % QR_N} y={Math.floor(i / QR_N)} width={1} height={1} fill="#0D3D3A" /> : null
      )}
    </svg>
  );
}
...
function StatusBar({ time }: { time: string }) {
  return (
    <div className="flex-shrink-0 flex items-center justify-between px-5 bg-white" style={{ height: 44, paddingTop: 6 }}>
      <span className="text-xs font-bold tabular-nums" style={{ color: "#0D3D3A" }}>{time}</span>
      <div className="flex items-center gap-1.5">
        <svg width="15" height="11" viewBox="0 0 15 11" fill="none">
          <path d="M7.5 8.5a1 1 0 1 1 0 2 1 1 0 0 1 0-2z" fill="#0D3D3A"/>
          <path d="M4.3 7.2a4.5 4.5 0 0 1 6.4 0" stroke="#0D3D3A" strokeWidth="1.2" strokeLinecap="round" fill="none"/>
          <path d="M1.5 4.5a8 8 0 0 1 12 0" stroke="#0D3D3A" strokeWidth="1.2" strokeLinecap="round" fill="none"/>
        </svg>
        <svg width="22" height="12" viewBox="0 0 22 12" fill="none">
          <rect x="0.5" y="0.5" width="18" height="11" rx="2.5" stroke="#0D3D3A" strokeWidth="1"/>
          <path d="M19.5 4v4a2 2 0 0 0 0-4z" fill="#0D3D3A"/>
          <rect x="2" y="2" width="13" height="8" rx="1.5" fill="#0D3D3A"/>
        </svg>
      </div>
    </div>
  );
...
export default function AttendeeView() {
  ...
  return (
    <div
      ...
      style={{
        background: "radial-gradient(ellipse at 50% 40%, #1a4a46 0%, #0D3D3A 55%, #061d1b 100%)",
      }}
    >
      ...
      <div className="flex-1 flex items-center justify-center py-4">
        ...
        >
          ...
          {/* Screen */}
          <div className="absolute flex flex-col overflow-hidden" style={{ top: 12, left: 12, right: 12, bottom: 12, borderRadius: 32, backgroundColor: "#F4FAFA" }}>
            <StatusBar time={clock} />

            {/* App header */}
            <div className="flex-shrink-0 px-5 py-3 bg-white" style={{ borderBottom: "1px solid #e8f0f0" }}>
              <div className="font-bold text-base" style={{ color: "#0D3D3A" }}>Tech Summit 2025</div>
              <div className="text-[11px] mt-0.5" style={{ color: "#8aabaa" }}>Hall A · Doors open 9:00 AM</div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto hide-scrollbar px-4 py-4 flex flex-col gap-4">
              ...
              {/* Attendee picker */}
              <div>
                <label className="text-[10px] font-bold uppercase tracking-wider block mb-1.5" style={{ color: "#9bbaba" }}>
                  Playing as
                </label>
                <select
                  value={selectedId}
                  ...
                  style={{ border: "1px solid #e0eeee", color: "#0D3D3A" }}
                >
              ...
              {/* Ticket card */}
              <div className="rounded-2xl overflow-hidden" style={{ boxShadow: "0 4px 20px rgba(13,61,58,0.12)" }}>
                {/* Ticket header */}
                <div className="flex items-center justify-between px-4 py-3" style={{ backgroundColor: "#0D3D3A" }}>
                ...
                <div className="flex bg-white">
                  ...
                  {/* Left: QR code */}
                  <div className="flex items-center justify-center p-3" style={{ borderRight: "1px solid #e8f0f0" }}>
                    <TicketQR dim={88} />
                  </div>

                  {/* Right: info */}
                  <div className="flex-1 px-4 py-4">
                    <div className="font-black text-base leading-tight mb-0.5" style={{ color: "#0D3D3A" }}>
                      {attendee.name}
                    </div>
                    <div className="text-xs mb-3" style={{ color: "#8aabaa" }}>{attendee.org}</div>
                    <span className="text-[11px] font-bold px-2.5 py-1 rounded-full" style={{ backgroundColor: colors.fill, color: colors.bg }}>
                      {attendee.type}
                    </span>
                    <div className="mt-2 text-[10px]" style={{ color: "#9bbaba" }}>
              ...
              <AnimatePresence mode="wait">
                {!isCheckedIn ? (
                  <motion.button
                    ...
                    className="w-full py-4 rounded-2xl text-base font-black text-white transition-opacity active:opacity-80"
                  ...
                  <motion.div
                    ...
                    style={{ backgroundColor: "#E1F5EE", border: "1px solid #a3e4cc" }}
                  >
                    <svg viewBox="0 0 64 64" width={52} height={52}>
                      <motion.circle cx={32} cy={32} r={29} fill="#fff" stroke="#1D9E75" strokeWidth={2.5}
                        initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ duration: 0.3, type: "spring" }} />
                      <motion.path d="M17 32l11 11L47 22" stroke="#1D9E75" strokeWidth={4.5} fill="none"
                        strokeLinecap="round" strokeLinejoin="round"
                        initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.35, delay: 0.2 }} />
                    </svg>
                    <div>
                      <div className="font-bold text-base" style={{ color: "#0d5c42" }}>
                        {attendee.type === "VIP" ? "Welcome, VIP!" : "You're checked in!"}
                      </div>
                      <div className="text-xs mt-0.5" style={{ color: "#1D9E75" }}>
              ...
              {/* Event info */}
              <div className="rounded-2xl p-4 bg-white" style={{ boxShadow: "0 1px 8px rgba(13,61,58,0.07)" }}>
                <div className="text-xs font-bold mb-3" style={{ color: "#0D3D3A" }}>Event Details</div>
                {[
                  ...
                  <div key={item.label} className="flex items-center gap-3 py-1.5" style={{ borderBottom: "1px solid #f0f8f7" }}>
                    <span className="text-sm w-5 text-center">{item.icon}</span>
                    <div>
                      <div className="text-[10px]" style={{ color: "#9bbaba" }}>{item.label}</div>
                      <div className="text-xs font-medium" style={{ color: "#0D3D3A" }}>{item.val}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {isCheckedIn && (
                <motion.div
                  ...
                  style={{ boxShadow: "0 4px 16px rgba(13,61,58,0.1)" }}
                >
                  <div className="px-4 py-2.5 flex items-center justify-between" style={{ backgroundColor: "#0D3D3A" }}>
                  ...
                  <div className="flex bg-white">
                    <div className="w-2 flex-shrink-0" style={{ backgroundColor: colors.stripe }} />
                    <div className="flex-1 px-4 py-4">
                      <div className="font-black text-lg" style={{ color: "#0D3D3A" }}>{attendee.name}</div>
                      <div className="text-xs mb-2" style={{ color: "#8aabaa" }}>{attendee.org}</div>
                      <span className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ backgroundColor: colors.fill, color: colors.bg }}>
                        {attendee.type}
                      </span>
                    </div>
                  </div>
                  <div className="px-4 py-2 flex items-center gap-2" style={{ backgroundColor: "#F4FAFA", borderTop: "1px solid #E8F6F5" }}>
                    <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: "#02C39A" }} />
                    <span className="text-[10px] font-medium" style={{ color: "#5a7a78" }}>Checked in · {record.time}</span>
                  </div>
                </motion.div>
              )}
